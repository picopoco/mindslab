...
class testDA(provider_pb2.DialogAgentProviderServicer):
    ...
    provider.description = 'english_writing'
    ...
    is_external = False
    stat = {}
    model_list = ['english_writing1','english_writing2','english_writing3','english_writing4','english_writing5']
    ...
    def GetRuntimeParameters(self, empty, context):
        ...
        sds_path.default_value = 'english_writing'
	...
	if talk.text.find('english_writing') == 0:
            local_stat.c_model = str(talk.text[-1])

        dp.model = 'english_writing' + local_stat.c_model
	...
	if Dialog.status == 'end':	# 한 session 마무리 시 다음 session으로 이동
	    ...
	    talk_res.text = "Congratulations, You did it!!"
	    talk_res.state = provider_pb2.DIAG_CLOSED
	return talk_res	
